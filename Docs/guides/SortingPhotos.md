...
#### Forcing a specific date (`concreteDate`)
Set `concreteDate` to force the same date onto every file. There is no option flag —
providing the date is the trigger, and it takes precedence over `.fixMetadata`:
```swift
...
#### Disposition (`.move` / `.keepOriginal`)
Disposition is **not** an option flag — it is a separate `SorterEngine.SourceDisposition`
parameter, `.move` by default. Use `.keepOriginal` to copy and leave originals untouched.
...
Files are **not** relocated into `outputFolder` — they stay in their original folder
(renamed in place if `.renameFiles` is set). The `outputFolder` is only used when
`.createFolders` is enabled. Use this mode for in-place renaming or in-place metadata fixing:
```
inputFolder/                     inputFolder/
├── IMG_1234.jpg      →          ├── 2024-01-15 10-30.jpg
├── IMG_1235.jpg                 ├── 2024-01-15 10-31.jpg
└── VID_0001.mov                 └── 2024-01-15 10-30.mov
```
...
let result = try await sorter.run(
    progressHandler: { progress in
        ...
        case .fileSkipped(let path):
            print("Skipped (already dated): \(path)")
...
errorHandler: { error in
    ...
    case .copyFailed(let source, let destination, let reason):
        // Could not copy file (in .keepOriginal mode)
...
PhotoSorterCore automatically handles duplicates by appending a `_N` suffix:

```swift
// If target file exists:
// 2024-01-15 10-30.jpg
// 2024-01-15 10-30_1.jpg
// 2024-01-15 10-30_2.jpg
```

### 5. Preserve Original Files

Use `.keepOriginal` disposition for non-destructive sorting — the input collection is
left untouched and copies are written to the output folder:

```swift
let config = SorterEngine.Configure(
    inputFolder: URL(fileURLWithPath: "/original/photos"),
    outputFolder: URL(fileURLWithPath: "/sorted/photos"),
