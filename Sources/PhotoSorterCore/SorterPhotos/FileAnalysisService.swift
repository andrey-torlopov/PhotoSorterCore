...
public final class FileAnalysisService {
    ...
    public func analyze(fileInfo: FileInfo) -> FileAnalysisResult {
        ...
        // Already named by date: the name contains a date AND it matches the metadata date.
        let isAlreadyNamedByDate: Bool = {
            guard let dateComponents = dateComponents else { return false }
            guard nameValidator.extractDateFromFileName(fileInfo.fileName) != nil else { return false }
            return nameValidator.validateDateMatch(fileName: fileInfo.fileName, metadataComponents: dateComponents)
        }()
        ...
        return FileAnalysisResult(
            ...
            isAlreadyNamedByDate: isAlreadyNamedByDate
        )
    }
}
