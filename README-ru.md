...
// `disposition` определяет, что делать с исходными файлами:
//   .move          → файлы перемещаются, оригиналы удаляются (по умолчанию)
//   .keepOriginal  → файлы копируются, оригиналы остаются нетронутыми
...
### Сохранение оригиналов (неразрушающая сортировка)

По умолчанию сортировщик **перемещает** файлы. Чтобы оставить входную коллекцию
нетронутой и писать копии в выходную папку, используйте `.keepOriginal`. В этом режиме
оригиналы — включая их метаданные — никогда не изменяются; любое исправление
даты/метаданных применяется только к копии.
...
### SorterEngine
Основной движок для сортировки и организации медиафайлов.

**Диспозиция источника** (`SorterEngine.SourceDisposition`, явная, по умолчанию `.move`):
- `.move` - Перемещение файлов; оригиналы удаляются из входной папки
- `.keepOriginal` - Копирование файлов; оригиналы (и их метаданные) остаются нетронутыми

**Опции** (булевы переключатели, комбинируются):
- `createFolders` - Создание структуры папок `Photos|Videos / Год / Месяц`
- `renameFiles` - Переименование файлов по формату даты
- `fixMetadata` - Исправление метаданных во время сортировки (применяется к итоговому файлу)
- `skipExistingDates` - Пропуск файлов, чьё имя уже соответствует валидной дате метаданных

Чтобы принудительно установить конкретную дату на каждый файл, задайте `concreteDate`
в конфигурации (он имеет приоритет над `fixMetadata`). Отдельного флага для этого нет.
