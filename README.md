...
// `disposition` controls what happens to the source files:
//   .move          → files are relocated, originals removed (default)
//   .keepOriginal  → files are copied, originals left untouched
...
### Preserve Originals (non-destructive sorting)

By default the sorter **moves** files. To keep the input collection intact and write
copies into the output folder, use `.keepOriginal`. In this mode the originals — including
their metadata — are never modified; any date/metadata fixing is applied to the copy only.
...
### SorterEngine
The core engine for sorting and organizing media files.

**Source disposition** (`SorterEngine.SourceDisposition`, explicit, default `.move`):
- `.move` - Relocate files; originals are removed from the input folder
- `.keepOriginal` - Copy files; originals (and their metadata) are left untouched

**Options** (boolean toggles, combinable):
- `createFolders` - Create `Photos|Videos / Year / Month` folder structure
- `renameFiles` - Rename files with the configured date format
- `fixMetadata` - Fix file metadata dates during sorting (applied to the resulting file)
- `skipExistingDates` - Skip files whose name already matches a valid metadata date

To force a specific date onto every file, set `concreteDate` in the configuration
(it takes precedence over `fixMetadata`). There is no separate flag for it.
